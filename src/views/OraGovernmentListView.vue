    ...
    .then((res: any) => {
      ...
      if (data?.status === 'OK') {
        ...
        if (data?.data.length > 4) {
          ...
          endPoint.value = data?.data.length - 3
        }
  ...
  <div class="OrganizationListView">
    ...
    <div class="OrganizationListView__content">
      ...
      <div class="navigate mb15px">
        <div @click="router.push({ path: '/finance-list' })" class="navigate__btn__disabled">
          금융기관 리스트
        </div>
        <div class="navigate__btn__enabled">지원기관 리스트</div>
      </div>
